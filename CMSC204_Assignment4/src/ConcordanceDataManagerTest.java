...
public class ConcordanceDataManagerTest {
	...
	public void testCreateConcordanceArraySTUDENT() {
		...
		text = "I like to eat pizza"
				+ "\nPizza is good and I eat it with coke"
				+ "\nCoke is not good for you, but it tastes like so good";

		ArrayList<String> words = concordanceManager.createConcordanceArray(text);
		assertEquals(words.get(0),"but: 3\n");
		assertEquals(words.get(1),"coke: 2, 3\n");
		assertEquals(words.get(2),"eat: 1, 2\n");
		assertEquals(words.get(3),"for: 3\n");
		assertEquals(words.get(4),"good: 2, 3\n");
		assertEquals(words.get(5),"like: 1, 3\n");
		assertEquals(words.get(6),"not: 3\n");
		assertEquals(words.get(7),"pizza: 1, 2\n");
		assertEquals(words.get(8),"tastes: 3\n");
		assertEquals(words.get(9),"with: 2\n");
		assertEquals(words.get(10),"you: 3\n");
	}
	...
	public void testCreateConcordanceFileB() {
		...
		try {
			...
			//sorted array has apostrophe before the other word
	...
	public void testCreateConcordanceFileSTUDENT() {
		...
		try {
			inputFile = new File("Tes1t.txt");
			PrintWriter inFile = new PrintWriter(inputFile);
			inFile.print("The ones who are crazy enough\n"
			...
			assertEquals("are: 1, 3", words.get(0));
			assertEquals("can: 2", words.get(1));
			assertEquals("change: 2", words.get(2));
			assertEquals("crazy: 1", words.get(3));
			assertEquals("enough: 1", words.get(4));
			assertEquals("jobs: 5", words.get(5));
			assertEquals("ones: 1, 3", words.get(6));
			assertEquals("quote: 4", words.get(7));
			assertEquals("steve: 5", words.get(8));
			assertEquals("that: 2", words.get(9));
			assertEquals("they: 2", words.get(10));
			assertEquals("think: 2", words.get(11));
			assertEquals("who: 1, 3", words.get(12));
			assertEquals("world: 2", words.get(13));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (Exception e) {
